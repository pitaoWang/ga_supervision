                ...
                <div style="width: 1280px;  margin-top: 10px; display: inline-block;background: white">
                    <Form ref="employeeval" :model="employee" :label-width="150" :rules="employeeValidate">
                        <div style="width:880px; float:left;">
                            ...
                            <Row>
                                <Col span="10">
                                    <FormItem label="人员类型" style="width: 350px;float: left;" prop="">
                                        <!--<Input v-model="employee.jobTitle" />-->
                                        <Select v-model="employee.jobTitle" multiple @on-change="selectop" placeholder="请选择用户类型">
                            ...
                            <Row>
                                ...
                                <Col span="9">
                                    <FormItem label="职务" style="width: 350px;" prop="techpos">
                                        ...
                                        <Input v-model="employee.techpos" placeholder="请输入"></Input>
                                    </FormItem>
                                </Col>
                            </Row>
                            <!--<Row>
                        ...
                        <div style="float:left; width:300px; margin-left: -50px">
                            <word v-on:childurlone="urldata" ref="ddd"></word>
                        </div>
...
<script>
    ...
    import awardRecord from './awardRecordlist.vue';
    ...
    export default {
        ...
        mounted:function(){
            let self = this;
            if(self.$route.query.employeeId){
                var id=self.$route.query.employeeId;
                self.$http.httpGet('/eim/api/employee/'+id, {
                }).then(function (response) {
                    let datainfo = response.data.data;
                    self.employee=datainfo;
                    self.$refs.ddd.urlupdate(datainfo);
                    self.employee.jobTitle =datainfo.jobTitle.split(',');
            ...
            if(self.$route.query.employee){
                var employee = self.$route.query.employee;
                this.employee=employee;


               // let userId=userdata.id;
               // self.companyName=self.$route.query.cname;

                //走后台获取employee
                /*self.$http.httpGet('/eim/api/employee/search/byUserId', {
                    userId,
                }).then(function (response) {
                    let datainfo = response.data.data;
                    self.employee=datainfo;
