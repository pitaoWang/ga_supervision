    ...
    <div class="container_div">
        <Row>
            <Col>
                    ...
                    <Row>
                        <Table :loading="loading"  :columns="columns" :data="data" sortable="custom" @on-sort-change="changeSort" @on-selection-change="showSelect" ref="table"></Table>
        ...
        <Modal :title="modalTitle" v-model="userModalVisible" :mask-closable='false' :width="500" :styles="{top: '30px'}">
            <Form ref="userForm" :model="userForm" :label-width="70" :rules="userFormValidate">
                ...
                <FormItem label="微信" prop="wechat">
                    <Input v-model="userForm.wechat"/>
                </FormItem>
               <!-- <FormItem label="员工类型" prop="">
                    &lt;!&ndash;<Input v-model="userForm.jobTitle"/>&ndash;&gt;
                ...
                <Form-item v-show="false" label="头像" prop="avatar">
  ...
  methods: {
        ...
        if (valid) {
          if (this.modalType === 0) {
            ...
            this.submitLoading = true;
              ...
              params.append('loginId', this.userForm.username);
              params.append('mobile', this.userForm.mobile);
              params.append('password', this.userForm.password);
              params.append('wechat', this.userForm.wechat);
              getUser(this.userForm.username).then(res => {
                  ...
                  if(res.succ===true){
                      ...
                      addUsers(params).then(res => {
                          ...
                          if(typeof (res.response)!="undefined"){
                              ...
                              this.userForm.employeeId = res.employeeId
    ...
    enable(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
            enableUsers(v.employeeId).then(res => {
                if (res.succ === true) {
    ...
    disable(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
            disableUsers(v.employeeId).then(res => {
                if (res.succ === true) {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
            delectUser(v.employeeId).then(res => {
